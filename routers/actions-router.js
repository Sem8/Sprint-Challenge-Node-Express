...
actionsRouter.get("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: `Error occurred while retrieving actions: ${error}` });
  }
});
...
actionsRouter.post("/", async (req, res) => {
  ...
  if (!project_id || !description || !notes) {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: `Error occurred while creating action: ${error}` });
    }
  }
});
...
actionsRouter.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `Error occurred while deleting action: ${error}` });
  }
});
...
actionsRouter.put("/:id", async (req, res) => {
  ...
  if (!project_id || !description || !notes) {
    res
      .status(400)
      .json({ message: "Needs a description, project Id or notes" });
  }
  try {
    const action = await Actions.get(project_id);
    if (!action) {
    ...
    res
      .status(500)
      .json({ error: `Error occurred while updating action: ${error}` });
  }
});
