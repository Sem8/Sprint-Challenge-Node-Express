...
router.post("/", async (req, res) => {
  ...
  if (!name || !description) {
    res
      .status(400)
      .json({
        message: "Bad request, submit name and description of new project."
      });
  } else {
    try {
      ...
      res
        .status(500)
        .json({ error: `Error occurred while creating project: ${error}` });
    }
  }
});
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `Error occurred while deleting project: ${error}` });
  }
});
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `Error occurred while updating project: ${error}` });
  }
});
